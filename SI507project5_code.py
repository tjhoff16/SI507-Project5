...
DEBUG = True
CACHE_FNAME = "tumblr_cache_contents.json"
CREDS_CACHE_FILE = "tumblr_creds.json"
...
### Private data in a hidden secretdata.py file
CLIENT_KEY = secretdata.client_key # what Twitter calls Consumer Key
CLIENT_SECRET = secretdata.client_secret # What Twitter calls Consumer Secret
...
class Post():
    def __init__(self, ID, likes, reblogs, tags):
        self.id = ID
        self.likes = likes
        self.reblogs = reblogs
        self.tags = tags
        self.engagements = likes + reblogs
...
if __name__ == "__main__":
    if not CLIENT_KEY or not CLIENT_SECRET:
        print("You need to fill in client_key and client_secret in the secretdata.py file.")
        exit()
...
hp_tumblr_base_search_url = "https://api.tumblr.com/v2/blog/funnyharrypotter.tumblr.com/posts"
tumblr_params_search = {'type':"text", "notes_info": True, "api_key": CLIENT_KEY}
tumblr_result = get_data_from_api(hp_tumblr_base_search_url,"Tumblr",tumblr_params_search) # Default expire_in_days
...
for post in tumblr_result['response']['posts']:
    ...
    for e in post['notes']:
        if e['type'] == 'like':
            ...
            reblogs +=1
    p = Post(pid, likes, reblogs, ptags)
    post_data.append(p)

with open("ind_post_data.csv",'w') as fl:
    fl.write("Post ID, Post Reblogs, Post Likes\n")
    for e in post_data:
        fl.write("{}, {}, {}\n".format(e.id, e.reblogs, e.likes))

with open("post_tags_data.csv",'w') as fl:
    fl.write('Tag, Times Used\n')
